...
router.post("/extract", async (req, res) => {
  ...
  try {
    ...
    if (status === 404) {
      return res.status(404).json({ error: "Không tìm thấy trang web (404)." });
    }
    if (status === 403 || status === 401) {
      return res.status(403).json({ error: "Trang web từ chối truy cập (403)." });
    }
    if (error.code === "ECONNABORTED" || error.code === "ETIMEDOUT") {
      return res.status(504).json({ error: "Hết thời gian chờ khi tải trang web." });
    }
    res.status(500).json({
...
function extractTextFromHtml(html: string): string {
  ...
  const blockTagRegex = new RegExp(`<\\/?(?:${blockTags.join("|")})[^>]*>`, "gi");
