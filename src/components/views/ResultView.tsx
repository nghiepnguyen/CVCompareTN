            ...
            <div className="space-y-6" id="analysis-result">
              <div className="flex items-center justify-between mb-8">
                <button 
                  ...
                  className="flex items-center gap-2 text-text-muted hover:text-accent font-bold text-sm transition-all group cursor-pointer hover:scale-105 active:scale-95"
                >
                  <div className="w-8 h-8 rounded-full bg-surface-secondary flex items-center justify-center group-hover:bg-accent-light transition-colors">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right hidden sm:block">
                    <div className="text-[10px] font-black text-text-light uppercase tracking-widest mb-1">{t.analysisTime}</div>
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-xl font-bold text-text-main">
                    {t.detailedAnalysis} <span className="text-accent">{selectedResult.jobTitle || selectedResult.cvName}</span>
                  </h3>
                </div>
                {selectedResult.jdTitle && (
                  <div className="flex items-start gap-2 ml-7 mt-2 p-3 bg-surface-secondary rounded-xl border border-border">
                    <FileSearch className="w-4 h-4 text-text-light mt-0.5 shrink-0" />
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] font-black text-text-light uppercase tracking-wider">{t.comparisonJD}</span>
                      <p className="text-xs text-text-muted leading-relaxed italic whitespace-pre-line line-clamp-5 break-all">
                        {selectedResult.jdTitle}
                      </p>
                    </div>
                  </div>
              ...
              <div id="tab-navigation" className="sticky top-16 z-40 bg-surface/90 backdrop-blur-xl py-4 -mx-4 px-4 mb-10 border-b border-border flex items-center justify-between shadow-sm transition-all duration-500">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        resultTab === tab.id ? "text-accent" : "text-text-muted hover:text-text-main"
                      )}
                    >
                      {resultTab === tab.id && (
                        <motion.div
                          layoutId="activeTab"
                          className="absolute inset-0 bg-surface shadow-xl shadow-accent-light border border-border rounded-[2rem]"
                ...
                <motion.div 
                  className="absolute bottom-0 left-0 h-[2px] bg-accent shadow-[0_0_8px_rgba(79,70,229,0.5)]"
                  initial={{ width: "25%" }}
                  animate={{ 
