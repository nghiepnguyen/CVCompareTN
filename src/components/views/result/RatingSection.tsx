...
export function RatingSection({ userId, analysisId }: RatingSectionProps) {
  ...
  if (isRatingSubmitted) {
    return (
      <div className="bg-success-light border border-success-light rounded-3xl p-8 text-center animate-in zoom-in duration-500">
        <div className="w-16 h-16 bg-surface rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-sm">
          <CheckCircle2 className="w-8 h-8 text-success" />
        </div>
        <h4 className="text-xl font-bold text-success mb-2">{t.thankYouRating}</h4>
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface rounded-3xl border border-border shadow-sm p-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
        <div>
          <h4 className="text-xl font-bold text-text-main mb-1">{t.rateResults}</h4>
          <p className="text-sm text-text-muted">{t.rateResultsDesc}</p>
        </div>
      ...
      <div className="space-y-4">
        <div className="relative">
          <MessageSquare className="absolute left-4 top-4 w-5 h-5 text-text-light" />
          <textarea
            value={userFeedback}
            onChange={(e) => setUserFeedback(e.target.value)}
            placeholder={t.feedbackPlaceholder}
            className="w-full pl-12 pr-4 py-4 bg-surface-secondary border border-border rounded-2xl text-sm focus:ring-2 focus:ring-accent focus:bg-surface transition-all min-h-[120px] text-text-main"
          />
        </div>
