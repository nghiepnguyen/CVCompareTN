...
5.  **SEO:** Pre-hydration `<script>` trong `index.html` (title, OG, hreflang vi/en/x-default, Schema.org); `AppContent.tsx` đồng bộ sau SPA navigation; sitemap 14 URL; legacy paths redirect 301; `scripts/generate-static-pages.ts` render sẵn nav + nội dung text vào `#root` cho từng route/locale (và cho `dist/index.html` fallback) để agent không chạy JS vẫn thấy content + outgoing links — 2026-07-18 fix Ahrefs "Page has no outgoing links", 2026-09-22 fix verify branding Google Auth Platform (bỏ clip-hidden, bọc `<!--email_off-->` chống Cloudflare che email). Chi tiết: [3_frontend.md](3_frontend.md).
