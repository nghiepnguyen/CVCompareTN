...
-   **`UIContext.tsx`**: Tab, ngôn ngữ, modals, nhãn UI. Hỗ trợ tab `'recruiter'`. Không dùng router library — tự đồng bộ `activeTab` ↔ URL qua `window.history.pushState`/`popstate` (`tabFromPath()`). **2026-07:** thêm URL riêng cho `history`/`recruiter`/`admin` (`/{lang}/history`, `/{lang}/recruiter`, `/{lang}/admin`) để hỗ trợ refresh, back/forward, deep-link — trước đó 2 tab này chỉ là state client, F5 luôn rơi về `analyze`. Tab `analyze`/`profile` vẫn không có path riêng (mặc định `/{lang}/`).
...
-   **`useFileProcessor.ts`**: `cleanText()` (chuẩn hóa whitespace/line endings) + `processFile(file, uid)` (đọc File → base64 data-URL hoặc plain text cho Gemini pipeline). **Từ 2026-07:** PDF trích xuất được text vẫn ĐỒNG THỜI trả kèm file gốc để Gemini nhìn layout — nếu file ≤ `MAX_INLINE_BINARY_SIZE` (2MB) thì trả base64 inline (`pdfInlineData`), nếu 2–15MB (`MAX_STORAGE_UPLOAD_SIZE`) thì upload thẳng lên Storage bucket `cv-analyze-tmp` qua `uploadTempAnalysisFile(uid, file)` và trả path (`pdfStoragePath`) thay vì base64 — tránh phình JSON body vượt giới hạn Vercel. PDF dạng scan/ảnh (không extract được text) áp dụng cùng logic 2 bậc cho chính `data` (base64 inline hoặc `dataStoragePath`); chỉ hard-fail khi vượt 15MB. Ảnh (`.jpg/.png/.webp`) cũng theo 2 bậc tương tự.
...
-   **`src/services/`**:
    ...
    -   **Supabase:** `userService`, `historyService`, `cvService` (bucket Storage `cv-files` — upload/download/delete CV cho kho CV; **2026-07** thêm `uploadTempAnalysisFile`/`cleanupTempAnalysisFiles` — bucket riêng `cv-analyze-tmp` cho file tạm trong luồng phân tích, dọn sau khi dùng xong).
...
-   **PDF layout-aware (2026-07, cập nhật):** ngoài text đã extract, `useFileProcessor.ts` còn gửi kèm file gốc (≤2MB: base64 inline `pdfInlineData`; 2–15MB: upload `cv-analyze-tmp` + gửi `pdfStoragePath`, server tải về base64) để Gemini "nhìn" được layout thật (nhiều cột, bảng/hình ảnh, header/footer chứa contact info...) khi chấm `formatAssessment` — không chỉ dựa vào text đã bị làm phẳng.
...
-   **Static body prerender (`scripts/generate-static-pages.ts`):** App là Vite SPA (`createRoot().render()`, không SSR) — `#root` rỗng cho tới khi React hydrate, nên agent không chạy JS (Ahrefs mặc định, bot verify branding của Google Auth Platform) thấy 0 nội dung/0 outgoing link. Script build này (chạy sau `vite build`) inject nav (Home/About/Upgrade/Privacy/Terms/Support) + nội dung text thật của từng trang (kéo từ `src/translations`, không import component thật để tránh side-effect Supabase/Sentry/GA4 lúc build) vào `#root`. An toàn vì client dùng `createRoot` (replace hoàn toàn, không phải `hydrateRoot`) nên không có hydration mismatch.

    Output: `dist/{vi,en}/{route}/index.html` cho 6 route (`/`, `/about`, `/privacy`, `/terms`, `/support`, `/upgrade`) **và** `dist/index.html` — file fallback mà mọi URL không khớp rewrite về; trước 2026-09-22 file này bị bỏ sót nên `#root` rỗng hoàn toàn.

    **Khối prerender không được ẩn bằng CSS.** Bản cũ bọc trong `clip:rect(0,0,0,0)` 1px; Google coi text bị clip là cloaking và loại khỏi phép đếm nội dung, làm `/vi/privacy` bị đánh giá "does not have sufficient content". Hiện render thành `<div id="seo-static">` bình thường; `index.html` có head script gắn class `js` vào `<html>` + `<style>.js #seo-static{display:none}</style>` — chạy trước khi body được parse nên user có JS không thấy nhấp nháy, còn agent không chạy JS đọc được toàn bộ.

    **Cloudflare Email Address Obfuscation** đứng trước Vercel, tự đổi mọi email trong HTML thành link `/cdn-cgi/l/email-protection` chỉ JS giải mã được — bot đọc ra `[email protected]` thay vì `admin@cvfit.pro`. Helper `emailOff()` bọc `<!--email_off--> … <!--/email_off-->` quanh email trong trang privacy để Cloudflare bỏ qua vùng đó. Không cần chỉnh dashboard Cloudflare.

    > Sửa gì liên quan crawler thì **verify trên edge live**, không chỉ trên `dist/`: `curl -s https://cvfit.pro/vi/privacy | grep -c "admin@cvfit.pro"`. Hai lỗi trên đều vô hình khi mở bằng browser.
