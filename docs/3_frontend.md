...
-   **`src/lib/`**: Thư mục chứa các hàm tiện ích (`utils.ts`), Supabase (`supabase.ts`), **GA4 + consent** (`ga4.ts`).
-   **`src/components/layout/CookieConsentBanner.tsx`**: Banner đồng ý cookie trước khi tải Google Analytics.
-   **`src/services/`**:
    ...
    -   **Supabase:** `userService`, `historyService`.
