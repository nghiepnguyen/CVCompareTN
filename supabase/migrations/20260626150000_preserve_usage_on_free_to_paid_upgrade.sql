-- When upgrading free → pro/recruiter: carry over usage_count so the user sees
-- their real tally against the new (higher) limit (e.g. 10/100, not 0/100).
-- When renewing same plan (pro→pro, recruiter→recruiter): reset to 0 — the user
-- bought another period and gets a fresh quota.
-- Plan expiry → free is unchanged: sync_profile_usage_month already resets to 0.

-- =============================================================================
-- 1. activate_pro_plan — preserve count on free→paid upgrade
-- =============================================================================
...
COMMENT ON FUNCTION public.activate_pro_plan(uuid, bigint, integer, jsonb, text) IS
  'Claims pending payment then extends pro/recruiter. Preserves usage_count when upgrading from free; resets to 0 when renewing same paid plan.';

-- =============================================================================
-- 2. admin_set_user_plan — same logic for admin grants
-- =============================================================================
...
COMMENT ON FUNCTION public.admin_set_user_plan(uuid, text, integer) IS
  'Admin-only: set user plan. Free downgrade resets usage_count. Free→paid upgrade preserves usage_count. Paid renewal resets to 0.';
